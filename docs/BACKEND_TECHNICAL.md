...
1. **Bell realtime** — currently polls on open; add Supabase realtime.
2. **@mentions** — mention internal (`team_member`) / external (`client_contact`/portal) people on comments, stored as **structured rows** (not parsed from text), emitted from the existing RPC write paths. Planned tables: extend `notification`; add `notification_preference` (per-user × per-channel toggles).
3. **Task-system slices** — subtasks/checklists, client-facing task sharing (the portal), auto-spawn from templates, a gantt/timeline view.
4. **Finer permissions** — the `agency_admin` vs `agency_member` split is enforced for the Admin area; extend the `isAgencyAdmin` gate to other destructive actions (delete client, billing, manage team).

*Email delivery (deploy `notify-email` + the `notify_email_on_insert` webhook) — **done**, live via Resend.*
