...
**Authority:** where this disagrees with `CLAUDE.md` or older notes, this document and the numbered SQL in `migrations/` win. Current as of **migration 0033**. See also [`PROJECT_GUIDE.md`](./PROJECT_GUIDE.md) (the broader reference) and [`EMPLOYEE_HANDBOOK.md`](./EMPLOYEE_HANDBOOK.md) (non-technical).
...
migrations/                # numbered SQL (0001–0033) + pgTap *_test.sql  ← source of truth for the DB
...
| `team_member` | Agency staff directory (0005) | `agency_id`, `full_name`, `role` (free text job title), `email`, `user_id` (nullable → auth.users), `is_active`. Owners/RACI/ownership resolve against this by `full_name`/`user_id`. |
...
- **Email (Edge Function):** `supabase/functions/notify-email/index.ts` (Deno), triggered by a **Database Webhook** on `notification` INSERT. Deliver-only: resolves the recipient email (service-role `auth.admin.getUserById`) and sends via Resend; subject + body both from `record.body`; returns 200 on every path (no retry storms). Requires the verified Resend domain. **This is deployed separately from Vercel** (see §11) and is currently the one piece still pending wiring.
...
- Currently **0001–0033**. See `PROJECT_GUIDE.md` §15 for the one-line ledger of each.
...
1. **Email delivery** — deploy `notify-email` + wire the Database Webhook (the bell works; email doesn't until this is done).
2. **Bell realtime** — currently polls on open; add Supabase realtime.
3. **@mentions** — mention internal (`team_member`) / external (`client_contact`/portal) people on comments, stored as **structured rows** (not parsed from text), emitted from the existing RPC write paths. Planned tables: extend `notification`; add `notification_preference` (per-user × per-channel toggles).
4. **Task-system slices** — subtasks/checklists, client-facing task sharing (the portal), auto-spawn from templates, a gantt/timeline view.
5. **Finer permissions** — the `agency_admin` vs `agency_member` split is enforced for the Admin area; extend the `isAgencyAdmin` gate to other destructive actions (delete client, billing, manage team).
