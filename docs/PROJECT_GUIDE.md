...
**Status:** Internal build, actively developed. Last updated 2026-07-15 (migration 0057).
...
15. [Migration ledger (0001–0057)](#15-migration-ledger)
...
20. [Campaign management](#20-campaign-management)
...
migrations/                       # numbered SQL (0001–0057) + pgTap *_test.sql
...
| `content_item` | A planned post | `id`, `client_id`, `channel_id`, `title`, `content_type`, `scheduled_at`, `status`, `current_version_id` (→ content_version, FK added 0021), `post_group_id` (0055 — links posts split off the same original), `created_by`, `updated_at`. **Production metadata (0042):** `designer_id` (→ team_member **on delete set null** — directory ref, login not required), `design_status`, `drive_url`, `high_res_url`, `boost` (bool, default false), `ad_budget` (numeric), `date_posted` (date), `posted_url`. Written via `set_post_meta`, **never** `update_post` (see §16). |
...
| `content_item_channel` (join, 0054) | 0054 | PK `(content_item_id, channel_id)`, both **on delete cascade**, index on `(channel_id)`. A post → many of its client's channels. Backfilled from `content_item.channel_id`, which stays the denormalised **primary** (first in the set). **RLS read proxies content_item's floor** (`content_item_id in (select id from content_item)`); no write policy — writes via `set_post_channels`/`create_post`. |
...
| `campaign` (**agency-only**) | 0056, +brief/money/targets/approval 0057 | `id`, `agency_id` (not null), `client_id` (not null → client **on delete cascade** — campaigns are client work), `name` (not null), `objective` (nullable, CHECK: awareness/traffic/leads/conversions/sales), `phase` (not null, default `planning`, CHECK: planning/production/live/wrapped/closed), `start_date`, `end_date` (table CHECK `campaign_date_order`: start ≤ end when both present), `created_by`, `created_at`, `updated_at`. **0057 adds:** `brief` (text), `media_budget`, `fee` (both numeric — `fee` is the fixed campaign price, **internal**), `kpi_target_results`, `kpi_target_cost_per_result` (numeric; table CHECK `campaign_money_nonneg`: each ≥ 0 when present), `brief_approved_at` (timestamptz), `brief_approved_by` (uuid). RLS read = `is_agency_member(agency_id)` — **no client/portal path in this slice** (the client-facing view is a later slice on a separate milestone table, §20). Index `(client_id, phase)`. RPC-only writes. See **[§20 Campaign management](#20-campaign-management)** for the full model. |
| `task.campaign_id` / `content_item.campaign_id` | 0056 | Nullable FK → `campaign(id)` **on delete set null** (deleting a campaign never destroys the work it grouped), each indexed. Groups a task/post under a campaign. The **client-match integrity rule** is enforced in the RPCs (`create_task`/`update_task`/`set_post_meta`): a row's campaign must belong to the **same client** as the row (and a campaign task must have a client) — §20. |
...
| `notification` (recipient-scoped) | 0019, +`email`/`task_id` 0041 | `user_id` (recipient), `type` (`ready_for_review`/`approved`/`changes_requested`/`comment`/`task_assigned`/`task_status`/`mention`), `content_item_id` (→ content_item on delete cascade), `task_id` (→ task on delete cascade, 0041), `actor_id`, `body`, `email` (bool, default true — false = in-app only, 0041), `read_at` (null = unread), `created_at`. |
...
| `mention` (structured, **RPC-write**) | 0053 | `id`, `source_type` (`comment`/`internal_note`, CHECK), `source_id` (**no FK** — a `comment.id` or `internal_note.id`), `mentioned_user_id` (→ auth.users **on delete cascade**), `created_by` (→ auth.users **on delete set null**), `created_at`; unique `(source_type, source_id, mentioned_user_id)`. Structured @mention rows (not parsed from text), written inside `add_comment`/`add_internal_note`. **RLS on, no read policy in v1** — write-only audit; the UI shows @names from the text and notifies via `notification`. |
...
### Campaign management (0056–0057)
Agency-only campaigns grouping tasks + content under a shared objective, budget, brief, and phase lifecycle — a **Campaigns** section on the client page (list + create), a **campaign hub** at `/campaigns/[id]` (header with phase-advance, a Brief panel with money/KPI targets + reversible brief-approval showing who/when, and tasks/content panels), a **Campaign dropdown** in the task modal and the post drawer. The **intake gate** blocks advancing to production/live/wrapped without an approved brief. **No client-facing surface** (a later slice). Full model in **[§20](#20-campaign-management)**.

### Security (all pgTap-proven)
Content read floor (0015), content tables RPC-only (0016), client transitions (0017), media table + storage policies (0018), notification RLS (0019), revoke (0020), versioning guards (0021), client version filter (0022), media reorder authorisation (0024), asset-link read floor + RPC auth (0026), task RLS + RPC auth (0028), client_ownership RLS + RPC auth (0030), RACI admin-write auth (0032), member-role admin-write + last-admin lockout (0033), team-member edit/deactivate auth (0034), invite create/accept auth incl. the client→agency leak guard (0035), permanent-delete auth + reassign/cascade + RACI merge (0036), view-preference own-rows RLS (0037), reschedule auth + mark-posted state-machine guard (0038), internal-note RPC auth + per-row agency resolution incl. the **client-leak guard** (0039), client status-setter auth + value validation (0040), task-subscriber RLS + seeding/event-actor-exclusion + cross-tenant (0041), set_post_meta auth + no-fork + cross-tenant (0042), task value/invoice validation + 0041/0043 regression guards (0045), agency cost-per-hour admin-only setter (0046), agency_internal admin-only read RLS + relocated cost rate (0047), campaign CRUD auth + cross-tenant + delete-guard + date validation + campaign_id persistence + the client-match integrity rule + 0041/0043/0045 regression guards (0056), campaign brief/money/target persistence + validation + the intake gate + reversible approval + phase-preserve/all-null regressions (0057).
...
|---|---|---|---|
| `create_post` | `(client_id, channel_id?, title?, content_type?, scheduled_at?, body?, visual_content?, channel_ids uuid[]?) → uuid` | agency-for-client | Creates the item + v1, sets `current_version_id`. `channel_ids` (0054) validates each belongs to the client, writes the join set, and sets `channel_id` to the first. |
| `set_post_channels` (0054) | `(item_id, channel_ids uuid[]) → void` | agency-for-client (no client path) | Replaces the post's channel set (`content_item_channel`); requires ≥1, validates each belongs to the client; sets `channel_id` to the first. **No version fork, no status change.** |
| `split_post_channel` (0055) | `(item_id, channel_id) → jsonb` | agency-for-client (no client path) | Peels one channel off a multi-channel post (≥2 required; channel must be on the post) into a new **draft** sibling: copies the current body/visual, clones media to new paths (returns `{new_item_id, media:[{old_path,new_path}]}` for the storage copy), detaches the channel from the original (which **keeps its status**), and links both via `post_group_id`. |
...
| `set_post_meta` (0042, +`campaign_id` 0056) | `(id, designer_id?, design_status?, drive_url?, high_res_url?, boost=false, ad_budget?, date_posted?, posted_url?, campaign_id?) → void` | agency-for-client (no client path) | Production-metadata setter for the content grid / drawer Production details. **No version fork, no status change.** Validates `designer_id` is a team member of the post's agency; **`campaign_id` (0056)** must belong to the **same client** as the post (else raises). **Full overwrite** of all metadata columns (see §16); 0056 dropped the exact 0042 signature first (duplicate-function trap). |
...
| `add_comment` | `(item_id, body, mentions uuid[] = '{}') → uuid` | member of the client | Emits comment notifications. **Mentions (0053):** each id must be an agency member of the post's agency OR a portal user of the post's client (else raises); notifies them (`type 'mention'`). |
...
|---|---|---|---|
| `add_internal_note` | `(parent_type, parent_id, body, mentions uuid[] = '{}') → uuid` | agency member of the parent's agency | Validates `parent_type`/body; resolves agency (post→client→agency; task→agency_id); inserts `author_id = auth.uid()`. **Mentions (0053):** each id must be an agency member of the parent's agency — **never a client contact** (else raises); notifies via `_notify`/`_notify_task` (`type 'mention'`). |
...
| RPC | Signature | Auth | Notes |
|---|---|---|---|
| `create_task` | `(… 0043 params …, value?, value_client_visible?, invoice_status?, campaign_id?)` → uuid | agency (derives agency from membership) | Validates client / owner / content item belong to the agency. `content_item_id` (0031) links the task to a post. **Seeds subscribers + emits an assignment notification (0041).** Validates `estimated_hours >= 0`, `start_date <= due_date` (0043), `value >= 0` + `invoice_status` in set (0045); **`campaign_id` (0056)** must belong to the **same client** as the task (and a campaign task must have a client). Each param-adding migration (0031/0041/0043/0045/**0056**) **drops the exact prior signature then recreates the full body** (duplicate-function trap, §16) — 0056 is the FOURTH such rebuild. |
| `update_task` | `(task_id, … same fields incl. content_item_id, estimated_hours, start_date, value, campaign_id …) → void` | agency member of the task's agency | **Full replace** (sets `updated_at`); mark-complete re-sends all fields with `status = 'Complete'`; kanban drag re-sends with the new `status`. **Re-seeds subscribers on owner change; emits assignment/status notifications (0041)** comparing the pre-update owner/status. Same capacity validation (0043) + the campaign client-match rule (0056). |
...
| RPC | Signature | Auth | Notes |
|---|---|---|---|
| `create_campaign` | `(client_id, name, objective?, phase='planning', start_date?, end_date?, brief?, media_budget?, fee?, kpi_target_results?, kpi_target_cost_per_result?) → uuid` | agency-for-client | Name required; validates objective/phase whitelist, date-order, and money/targets ≥ 0. **Intake gate (create side):** a new campaign has no approved brief, so it cannot **start** in a gated phase (production/live/wrapped) — planning/closed only. 0057 dropped both the 0056 and the new signatures first (idempotency, §16). |
| `update_campaign` | `(id, name, objective?, phase?, start_date?, end_date?, brief?, media_budget?, fee?, kpi_target_results?, kpi_target_cost_per_result?) → void` | agency-for-client (via the campaign's client) | **Full-overwrite** of name/objective/dates/brief/money/targets; **phase alone PRESERVES** when omitted (the 0050 "preserve-don't-default" lesson, §16, reproduced verbatim). **Intake gate:** advancing INTO production/live/wrapped requires an approved brief — a **genuine transition only** (unchanged phase and planning→closed never gated). **Does NOT touch approval** (see `set_brief_approved`). 0057 dropped both old+new signatures. |
| `delete_campaign` | `(id) → void` | **agency_admin** of the campaign's agency | Two-step, mirrors `delete_client`: only a **`closed`** campaign is deletable. Grouped tasks/content survive via `campaign_id` → set null. |
| `set_brief_approved` (0057) | `(id, approved bool) → void` | agency-for-client (member level) | The **only** approval path — never `update_campaign`. Approving stamps `brief_approved_at = now()`, `brief_approved_by = auth.uid()`; un-approving **nulls both** (reversible — mistakes happen). |
...
| 0053 | mentions (+test) | `mention` table (polymorphic structured rows, write-only audit) + extends `add_comment` / `add_internal_note` with `p_mentions uuid[]`. Comment mentions allow the post's agency team OR the post's client contacts; internal-note mentions allow agency team ONLY (client-leak guard, pgTap-proven). Notifies via `_notify` / `_notify_task`, type `'mention'`. |
| 0054 | post_channels (+test) | `content_item_channel` join table (a post → many of its client's channels) + backfill from `channel_id`; `set_post_channels(item, channel_ids[])` replaces the set with no version fork or status change; `create_post` gains `p_channel_ids uuid[]`. `channel_id` retained as the denormalised primary (first in the set). RLS read proxies content_item's floor; writes RPC-only. |
| 0055 | split_post_channel (+test) | `content_item.post_group_id` (links split siblings) + `split_post_channel(item, channel) → jsonb` peels one channel off a multi-channel post into a new DRAFT post (copies current body/visual + clones media to new paths via the fork contract; detaches the channel from the original, which keeps its status). Agency-only. |
| 0056 | campaigns (+test) | **Campaign management slice 1.** `campaign` table (agency-only, client-scoped, objective/phase enums, date-order CHECK, `(client_id, phase)` index) + `campaign_id` on `task`/`content_item` (on delete set null, indexed). `create_campaign`/`update_campaign`/`delete_campaign` (admin + closed-only delete). Rebuilds `create_task`/`update_task` (drops the exact 0045 signatures, recreates the full bodies + `p_campaign_id` + the **client-match integrity rule**) and `set_post_meta` likewise — the FOURTH task-RPC rebuild. RLS = agency-only, **no client read path** (client surface is a later slice). See §20. |
| 0057 | campaign_brief (+test) | **Campaign management slice 2.** `campaign` gains `brief`, `media_budget`, `fee` (fixed internal price), `kpi_target_results`, `kpi_target_cost_per_result` (`campaign_money_nonneg` CHECK), and the reversible approval stamp `brief_approved_at`/`brief_approved_by`. Rebuilds `create_campaign`/`update_campaign` (**drops BOTH old and new signatures** — idempotency, §16 — preserving the 0050 phase-preserve rule verbatim) with the full-overwrite brief/money/target fields and the **intake gate** (approve-before-production, genuine-transition-only, planning→closed exempt). `set_brief_approved(id, approved)` is the **only** approval path (agency-member, reversible). See §20. |
...
- **The "preserve-don't-default" lesson (0050) — omit means keep, not reset.** A setter whose params default to a *concrete value* silently resets any field the caller doesn't resend. The status-revert — saving an unrelated field via the client edit form reverted an "Archived" client to "Active" — was root-caused: `update_client` defaulted `p_status`/`p_timezone`/`p_currency` to `'active'`/`'Europe/Malta'`/`'EUR'` and wrote `status = coalesce(p_status,'active')`, so an omitted field was reset. **0050** defaults those three params to **null** and coalesces each to the **existing value**, so an omitted field is preserved. This is now a **named, reusable rule**: a field that should survive an omitted write must default to null and coalesce to its current value — cited by `update_campaign`'s phase-preserve behaviour in **0056/0057** (`v_phase := coalesce(nullif(btrim(p_phase),''), <current phase>)`, whitelist-checked only when a value is supplied). `set_client_status(client_id, status)` stays the lightweight, single-field path for archive/reactivate — the clients-list actions still use it — and remains the right tool for a status-only change, since `update_client` still resends every other field.
...
- **Adding params to an RPC → drop the exact old signature first (the duplicate-function trap).** `create or replace function` only replaces when the argument signature matches. Adding a parameter changes the signature, so `create or replace` silently creates a **second overload** and leaves the old one — then PostgREST may resolve to the wrong one. When a migration adds params (e.g. 0031 task content link, **0043 capacity fields**, **0056 campaign_id**), it must `drop function if exists public.fn(<exact old arg types>)` first, then `create`. And it must **reproduce the full current body** (0043 re-pasted the whole 0041 `create_task`/`update_task` bodies — subscription seeding + events — so that behaviour wasn't lost; 0056 was the *fourth* such `create_task`/`update_task` rebuild). Verify live signatures via `pg_proc` before editing.
- **Ledger rule earned this fortnight — a rebuilt-function migration must drop BOTH the old AND the new signatures (idempotency).** A single-shot `drop <old>; create <new>;` is **not** re-runnable: run it twice and the second `create` throws **`42723` (function already exists)**, because the first run already left the new signature in place and the drop only targets the *old* one. So the rebuild block must `drop function if exists` **both** the pre-migration signature and the post-migration signature before `create` (0057's `create_campaign`/`update_campaign` do exactly this — the 0056 lesson). Single-shot migration files are no longer acceptable in this ledger; every param-changing rebuild ships both drops.
- **A post can target several channels (`content_item_channel`, 0054).** A post stays **one post with one approval** while its content is shared across many of its client's channels (the join table). `channel_id` is kept as the **denormalised primary** (first in the set) so existing single-channel reads keep working. Channels are edited via **`set_post_channels`** (no fork, no status change) — **not** `update_post` (the edit form no longer has a channel field). `create_post` can take the whole set up front via `p_channel_ids`. **Tailoring / split-on-divergence (0055):** when one channel needs to diverge, **`split_post_channel`** peels it into its own **draft** sibling (copying the current body/visual + media) and detaches it from the original; the two are linked by `post_group_id` and approve independently. Surfaced in the drawer's Channels section as a per-channel **Tailor** action, with a link glyph on cards/header marking split siblings.
...
- `throws_ok(sql, '<sqlstate>')` 2-arg form (3-arg mis-binds); `is_empty`/`isnt_empty` for silent RLS reads; `lives_ok(sql, desc)` for "does not error"; `auth.users` test rows need only `id` + `email`; wrap in `begin; … rollback;`.
- **Harness rule earned this fortnight — any temp table read while `set local role authenticated` is active needs an explicit grant.** The role switch drops superuser privileges, so a `select … from _ctx` **inside** an `is_empty`/`isnt_empty` (or any assertion) run under `authenticated` fails with **`42501` (permission denied for table _ctx)** unless you `grant select on _ctx, … to authenticated;` up front (alongside the familiar `grant insert on _t to authenticated;` for the results table). Same trap family bit **0039** (the `_t` insert grant) and **0056** (test 14 read `_ctx` under the role switch). Grant every temp table the role-switched block touches, right after you create them and before `plan()`.
...
Client Approve/Request-changes UI · snapshot-on-send versioning · agency + client version history · agency dashboard · calendar filters · month-view media indicator · enriched notification copy · persisted media ordering + drag-reorder · real portal revocation · **combined all-clients calendar** · **per-client `calendar_colour` + legend** · **labelled asset links (0026)** · **RACI reference data (0027)** · **internal task list + dashboard summary (0028)** · sidebar logo · **legacy `asset` table dropped (0029)** · **per-client ownership + matrix (0030)** · **content↔task bridge + Lead-PM owner suggestion (0031)** · **admin area + editable RACI matrix (0032)** · **task List/Kanban/Calendar views + deeper dashboard breakdowns** · **permission management: Admin → Team access, promote/demote with last-admin lockout (0033)** · **team member edit + soft-deactivate/reactivate (0034)** · **agency + client invites, magic-link-native, accept-on-login (0035)** · **permanent delete: reassign-then-delete team members, guarded-cascade clients (0036)** · **per-user column preferences on the task list (0037)** · **drag-to-reschedule posts on the calendar, agency-only, with a past-date confirm (0038)** · **internal notes on posts + tasks, agency-only, author-owns, one polymorphic table (0039)** · **archive / reactivate / delete actions in the clients list, with a lightweight `set_client_status` setter + type-the-name delete confirm (0040)** · **RACI-seeded task subscriptions + task event notifications with an in-app-only vs email flag (0041)** · **post production metadata + the agency content-grid view + drawer Production details, via the no-fork `set_post_meta` (0042)** · **task `estimated_hours` + `start_date` and a per-person capacity planner on the dashboard (week/month, vs 40h, with honesty buckets) (0043)** · **internal timesheets per client — DB-backed timer + manual logging (0044)** · **job value / client-visibility / invoice status on tasks (0045)** · **admin-only agency cost-per-hour (0046), hardened into an admin-only `agency_internal` table (0047)** · **admin-only profitability report at `/reports` (value − time-cost margins by client, date-ranged) (0047)** · **`update_client` preserves status/timezone/currency on omit (0050)** · **agreed client deliverables — agency-only CRM section + RPCs (0051)** · **global "+ Log time" top-bar modal (pick task / free-text / create-task-on-the-fly, Malta-correct, portaled)** · **/reports reopened to all agency members as a tabbed shell — member-visible Time + Capacity (relocated from the dashboard) tabs, admin-only Profitability tab** · **campaign management slice 1 — the `campaign` entity + `campaign_id` grouping on tasks/content with the client-match rule + the campaign hub (0056)** · **campaign management slice 2 — brief, media budget, fixed internal fee, KPI targets, and the approve-before-production intake gate with a reversible brief approval (0057)**.

---

## 20. Campaign management

A **campaign** is a named, client-scoped body of work (a launch, a seasonal push) that groups tasks and content under a shared objective, budget, brief, and lifecycle. Built in two slices — **0056** (the entity + grouping + hub) and **0057** (brief, budgets/fee, KPI targets, the intake gate). **Agency-internal throughout: there is no client-facing surface** — the client view is a later slice on a separate table (below). Full RPC signatures in [§14](#14-rpc-reference); columns in [§6](#6-data-model); ledger in [§15](#15-migration-ledger).

### The entity
`campaign` is **agency-only** (RLS `is_agency_member(agency_id)`, no client/portal path) and **client-scoped** (`client_id` not null, `on delete cascade` — campaigns are client work). Two enums:
- **`objective`** (nullable): `awareness` / `traffic` / `leads` / `conversions` / `sales`. Drives the KPI unit label (`lib/campaignConstants.ts` → "200 leads · ≤ €20/lead").
- **`phase`** (not null, default `planning`): the lifecycle below.

Money/targets (0057): `media_budget`, **`fee`** (the fixed campaign price), `kpi_target_results`, `kpi_target_cost_per_result` — each **≥ 0 when present** (`campaign_money_nonneg` CHECK + RPC validation raising P0001). Plus `brief` text and the approval stamp `brief_approved_at`/`brief_approved_by`.

### Phase lifecycle
```
planning ─▶ production ─▶ live ─▶ wrapped ─▶ closed
```
The hub's **phase-advance** button steps through this order one step at a time (`nextPhase`). **Delete** is admin-only and only from **`closed`** (mirrors the archived-only `delete_client` guard); grouped tasks/content survive via `campaign_id → set null`.

### Grouping + the client-match integrity rule
`task.campaign_id` and `content_item.campaign_id` (both nullable, `on delete set null`, indexed) group work under a campaign. The **integrity rule** is enforced in the RPCs, not just the UI: a row's campaign must belong to the **same client** as the row — `create_task`/`update_task` raise if `campaign.client_id ≠ task's client` **or the task has no client**; `set_post_meta` raises if `campaign.client_id ≠ the post's client`. The task modal's Campaign dropdown lists only the **selected client's** open campaigns (clears on client change); the post drawer's Production details hosts the post's campaign picker.

### The intake gate (approve-before-production)
Advancing a campaign **into `production` / `live` / `wrapped` requires an approved brief** (`brief_approved_at is not null`) — raised loudly ("approve the brief before production", surfaced **verbatim** on the hub's Advance button via P0001 prefix-stripping). Design decisions:
- **Enforced in `update_campaign`'s phase path**, not the UI alone — so any caller is gated. It fires on a **genuine transition only** (`v_phase is distinct from v_current_phase and v_phase in (production,live,wrapped)`), so editing an already-in-production campaign never trips it.
- **`planning → closed` is never gated** — an abandoned campaign never needed a brief.
- **Create side too:** a brand-new campaign has no approved brief, so `create_campaign` refuses to *start* in a gated phase (the client-page create form offers only planning/closed).
- A cleaner home would be a dedicated `advance_campaign_phase` RPC (flagged), but the gate lives with the phase logic per the brief.

**`set_brief_approved(id, approved)` is the ONLY approval path** — approval is *not* settable through `update_campaign`, so an edit save can never silently (un)approve. It's **agency-member level** and **reversible**: approving stamps `now()` + `auth.uid()`; un-approving nulls both. The hub shows *who / when* once approved.

### Field semantics — full-overwrite, phase-alone-preserves
`brief` / `media_budget` / `fee` / `kpi_*` / `name` / `objective` / dates follow the documented **full-overwrite** rule: the edit form always sends the complete current state; a missing value writes null. **Phase alone preserves** when omitted (the [§16 "preserve-don't-default" lesson](#16-conventions--hard-won-gotchas), 0050). Two consequences the code respects: the hub **phase-advance forwards the whole field set** (changing only the phase) so it never wipes the brief; the Brief edit form **resends name/objective/phase/dates as hidden fields** alongside the editable money/targets.

### `fee` is the member-visible tier, deliberately NOT the admin-only cost tier
`campaign.fee` is the **fixed campaign price** and lives on the member-visible `campaign` table — it is **not** the sensitive blended internal rate. That rate is `agency_internal.cost_per_hour` (**0047**, admin-only RLS, §7). The two tiers are kept separate on purpose: campaign fee and media budget are agency-member-visible (labelled "internal" in the UI to mark them agency-only vs client-facing), whereas the cost-per-hour that feeds profitability margins stays admin-only. When the client view lands (slice 4), the client sees **ad budget + spend, never the fee**.

### App surfaces (all agency-only)
- **Client page (`/clients/[id]`)** — a **Campaigns** section: a list (name, phase pill, dates, objective) + a create form (name / objective / phase [planning|closed] / dates / media budget / fee·internal / KPI targets / brief).
- **Campaign hub (`/campaigns/[id]`)** — header (name, client link, phase pill + **advance** control, objective, dates, admin **delete** when closed); a **Brief panel** (brief text, media budget, fee·internal, KPI one-liner) with edit + a **Mark brief approved** / un-approve control showing who + when; and **Tasks** + **Content** panels linking into the task board / post drawer.
- **Task modal** — a Campaign dropdown (selected client's open campaigns, clearable). **Post drawer Production details** — the post's Campaign picker (via `set_post_meta`).

### Decided-but-unbuilt slices
The roadmap agreed for campaigns (nothing below is built):
- **Slice 3 — Gantt + rollups** *(no migration)*: a timeline view over campaigns/tasks + budget/hours rollups per campaign (pure computation over existing data).
- **Slice 4 — Client view** *(separate milestone table)*: a **milestones-only** client-facing surface — the client sees campaign **ad budget + spend, never the `fee`**. Deliberately a *new* table so nothing client-visible couples to the agency-only `campaign` schema.
- **Slice 5 — Templates**: reusable campaign/task templates to spin up the standard set of work.
- **Slice 6 — Metrics table** *(source-agnostic)*: a results table keyed to a campaign, agnostic to where the numbers come from (manual entry or, later, sync).
- **Slice 7 — Meta sync**: pull results from Meta via a **system-user token** against **partnered ad accounts**.
- **Slice 8 — Google**: the same for Google, via an **MCC** (manager account).
...
| **Campaign** | A named, client-scoped, agency-only body of work (`campaign`, 0056/0057) grouping tasks + content under an objective, budget, brief, and phase lifecycle (planning→production→live→wrapped→closed). |
| **Intake gate** | The 0057 rule that advancing a campaign into production/live/wrapped requires an approved brief (`set_brief_approved`); genuine-transition-only, planning→closed exempt. |
| **Preserve-don't-default** | The 0050 setter lesson (§16): a field that should survive an omitted write defaults to null and coalesces to its current value — cited by `update_campaign`'s phase-preserve. |

---

*This document reflects the codebase at migration 0057. Keep it current: when you ship a feature or migration, update the relevant section, the migration ledger, and the open-items list.*
