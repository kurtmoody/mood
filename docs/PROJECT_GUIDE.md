...
15. [Migration ledger (0001–0047)](#15-migration-ledger)
...
migrations/                       # numbered SQL (0001–0047) + pgTap *_test.sql
...
- **Profitability report (0047)** — `/reports` (**agency-admin only**, gated like `/admin/costs`): per-job rows grouped by client — **Value** (full price, not date-split), **Cost** (in-range time × rate), **Margin**, **Margin %**, **Invoice** — with a per-client **"Unattributed time"** line (client-direct time, `task_id` null) so client totals reconcile, per-client subtotals + grand total, and per-client **outstanding to invoice** (sum of not-invoiced value). Date-range presets (day/week/month/quarter/year/custom, Malta). A **null-rate notice** when no cost rate is set (no fake 100% margins) and an **honesty caveat** (value isn't date-distributed). `lib/profitability.ts` + `lib/reportRange.ts` (pure); `ProfitabilityReport` component. **Margins live ONLY here** — the all-members `/dashboard` capacity view shows hours, never euros.
...
*This document reflects the codebase at migration 0047. Keep it current: when you ship a feature or migration, update the relevant section, the migration ledger, and the open-items list.*
